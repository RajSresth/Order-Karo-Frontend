...
const navLinksByRole = {
  user: [{ label: "My Orders", path: "/", icon: FiPackage }],

  owner: [
    { label: "Dashboard", path: "/", icon: FiGrid },
    { label: "My Shops", path: "/my-shops", icon: FiShoppingBag },
    { label: "Pending Orders", path: "/#pending-orders", icon: FiClock },
    { label: "Create Shop", path: "/create-shop", icon: FiHome },
    { label: "Add Item", path: "/create-item", icon: FiPlusCircle },
  ],

  admin: [{ label: "Dashboard", path: "/", icon: FiGrid }],

  rider: [{ label: "Dashboard", path: "/", icon: FiGrid }],
};
...
const Nav = () => {
  ...
  const roleLinks = navLinksByRole[role] || [];
  const isUser = role === "user";
  const locationText = [city, state, address].filter(Boolean).join(" ");
        ...
        <div className="flex items-center justify-between gap-6">
          ...
          {isUser && (
            <div className="grow flex px-4 py-2 items-center gap-6 max-w-2xl border border-orange-600 rounded-lg">
              <div className="flex items-center gap-2 text-gray-700 min-w-0">
                <FiMapPin className="text-orange-600 text-xl shrink-0" />
                <span className="text-sm truncate text-gray-500 font-medium max-w-40">
              ...
              <div className="grow border-orange-600 border-l-2">
                <div className="relative">
                  <input
                    type="text"
                    placeholder="Search restaurants, food..."
                    className="w-full px-4 py-1 pr-10 border-none outline-none text-gray-700"
                  />
                  <button className="absolute right-3 top-1/2 -translate-y-1/2 text-orange-600">
          ...
          <div className="flex items-center gap-5 shrink-0">
            ...
            {roleLinks.map((item) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-2 text-gray-700 font-medium hover:text-orange-600 transition whitespace-nowrap"
                >
            ...
            <div className="relative flex items-center gap-2">
              ...
              {isOpen && (
                <div className="absolute right-0 top-14 w-56 bg-white rounded-2xl shadow-2xl border border-gray-100 p-3 z-50 flex flex-col gap-1">
                  <div className="px-3 py-2 border-b border-gray-100 mb-1">
                    <p className="text-sm font-semibold text-gray-800">
                      {userData?.fullname || "User"}
                    </p>
                    <p className="text-xs text-gray-400">{userData?.email}</p>
                    <p className="text-xs text-orange-600 font-semibold capitalize mt-1">
                      {role}
                    </p>
                  </div>

                  <button className="flex items-center gap-3 px-3 py-2 rounded-xl text-gray-700 text-sm font-medium hover:bg-orange-50 hover:text-orange-600 transition text-left w-full cursor-pointer">
                    <FiUser className="text-base" />
                    Profile
                  </button>

                  <button
                    className="flex items-center justify-center gap-2 mt-1 w-full bg-orange-600 hover:bg-orange-700 text-white text-sm font-semibold py-2.5 rounded-xl transition shadow-sm hover:shadow-md active:scale-95 cursor-pointer"
                    onClick={handleLogout}
                  >
                    <FiLogOut className="text-base" />
                    Logout
